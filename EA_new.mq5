...
input ENUM_TRADE_DIRECTION TradeDirection = TRADE_BOTH;  // Trading direction (Buy/Sell/Both)
input int      Magic = 123456;         // EA identifier number

input group    "=== Position Sizing ==="
input double   Lots = 0.1;             // Base position size
input double   LotMultiplier = 1.5;    // Position size multiplier per trade
input double   MaxLot = 1.0;           // Maximum position size allowed

input group    "=== Entry Conditions ==="
input int      MinDistancePips = 30;   // Minimum distance between positions
input int      OpenTime = 60;          // Minimum time between positions
input int      PipsStep = 20;          // Required price movement for entry
input int      MaxSpread = 50;         // Maximum allowed spread

input group    "=== Trading Hours ==="
input int      TimeStart = 0;          // Trading session start hour
input int      TimeEnd = 23;           // Trading session end hour

input group    "=== Bollinger Bands Filter ==="
input bool     UseBollingerFilter = true;  // Enable/disable Bollinger filter
input int      BBPeriod = 20;          // Bollinger period
input double   BBDeviation = 2.0;      // Bollinger standard deviation
input ENUM_APPLIED_PRICE BBPrice = PRICE_CLOSE; // Price type for Bollinger

input group    "=== Exit Conditions ==="
input int      Tral = 40;             // Trailing stop distance
input int      TralStart = 15;        // Trailing stop activation distance
input double   TakeProfit = 30;       // Take profit level

input group    "=== Interface Settings ==="
input bool     Info = true;            // Show/hide info panel
...
void OnTick() {
   ...
   if(TradeDirection == TRADE_BUY_ONLY || TradeDirection == TRADE_BOTH) {
      bool bollingerCondition = !UseBollingerFilter || currentBid > upperBand;
      if(currentBid > lastBidPrice && totalPriceMovement >= PipsStep && bollingerCondition) {
         canOpenBuy = true;
         Print("Buy condition met - Price moved up by ", totalPriceMovement, " pips", 
               UseBollingerFilter ? " and above upper Bollinger band" : "");
      }
   }
   
   if(TradeDirection == TRADE_SELL_ONLY || TradeDirection == TRADE_BOTH) {
      bool bollingerCondition = !UseBollingerFilter || currentBid < lowerBand;
      if(currentBid < lastBidPrice && totalPriceMovement >= PipsStep && bollingerCondition) {
         canOpenSell = true;
         Print("Sell condition met - Price moved down by ", totalPriceMovement, " pips", 
               UseBollingerFilter ? " and below lower Bollinger band" : "");
      }
   }
...
void OpenBuyOrder() {
   ...
   // Check minimum delay
   datetime currentTime = TimeCurrent();
   if(currentTime - lastBuyPositionTime < OpenTime) {
...
void OpenSellOrder() {
   ...
   // Check minimum delay
   datetime currentTime = TimeCurrent();
   if(currentTime - lastSellPositionTime < OpenTime) {
